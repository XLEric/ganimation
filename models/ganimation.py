...
class GANimation(BaseModel):
    ...
    def _create_discriminator(self):
        return NetworksFactory.get_by_name('discriminator_wasserstein_gan',image_size = self._opt.image_size,c_dim=self._opt.cond_nc)
    ...
    def set_input(self, input):
        ...
        if len(self._gpu_ids) > 0:
            ...
            self._input_real_img = self._input_real_img.cuda(self._gpu_ids[0], non_blocking=False)
            self._input_real_cond = self._input_real_cond.cuda(self._gpu_ids[0], non_blocking=False)
            self._input_desired_cond = self._input_desired_cond.cuda(self._gpu_ids[0], non_blocking=False)
